...
function gatherHours(users: UserWithWorkingTimes[]) {
  const usersWorkingTimes = users.map((user) => {
      ...
      return {user, workedHours: totalHours}
  })

  return usersWorkingTimes
}

const data = ref<{ user: UserWithWorkingTimes; workedHours: number; }[]>([]);

onBeforeMount(async () => {
  // const team = await getTeam(teamId, {withUsers: true, withWorkingTime: true})


  // data.value = gatherHours(team.data.users)
...
<template>
  <Card title="Week summary" class="gap-x-6 items-center justify-between w-full md:w-[66%] lg:w-[50%] xl:[40%] max-w-[580px]">
    ...
    <div v-for="(row, index) in data" :key="index" class="py-4 px-4 w-full border-b last:border-0 border-slate-200 justify-between grid grid-cols-4 gap-x-3">
      <div class="col-span-1">{{ row.user.username }}</div>
